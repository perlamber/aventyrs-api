...
| CharacterSheet | `/api/character-sheets` | Embeds its `character` (name, race, sexo, alignment); references a `playerId`. New sheets start with zeroed stats, mirroring `CharacterSheet.of(...)` in core. Supports `?playerId=` filtering. Responses carry `campaignId` and `progressionLocked` (see Campaign); `PUT` never changes `campaignId` and does not enforce the lock |
| Campaign | `/api/campaigns` | Numbered Sessões (`CREATED`→`ONGOING`→`ENDED`, rules from core's `org.aventyrs.core.campaign`): `POST /{id}/sessions` adds the next one (`409` while another is still open), `POST /{id}/sessions/{n}/start` and `.../end` move it. `PUT`/`DELETE /{id}/participants/{sheetId}` set or clear a sheet's single `campaignId` (adding moves a sheet out of any other Campaign). `progressionLocked` is true while a Sessão is `ONGOING`, and the client is expected to block new Talentos, Graduações, Títulos, Especializações, Habilidades and Atributo raises while it holds. **The lock is client-side only.** Every Sessão or participant change broadcasts the Campaign on `/topic/campaigns/{id}/sessions`. There is no `PUT /{id}` |
